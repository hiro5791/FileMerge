...
| 各ファイルを行頭から始める | **OFF** | 改行で終わっていないファイルに改行を補う |
| 2個目以降の見出し行を除く | **OFF** | 2個目以降の1行目を落とす（CSV / TSV 向け） |
| 各ファイル末尾の空行を削除 | **OFF** | 末尾の空行を取り除く |
| 2個目以降の BOM を取り除く | **OFF** | 2番目以降のファイルの BOM を除去する |

既定のままだと望ましくない結果になりそうな場合は、**警告を出して1クリックの修正を提案**します。勝手に修正はしません。

- 「文字コードが混在しています（UTF-8 / Shift_JIS）。このまま結合すると一部が文字化けします。」→ **すべて UTF-8 に変換する**
- 「2 個目以降のファイルに BOM があります…」→ **BOM を取り除く**
- 「改行で終わっていないファイルがあります…」→ **改行を補う**
