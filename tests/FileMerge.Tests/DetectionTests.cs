...
public sealed class DuplicateNameResolverTests
{
    private static List<FileMerge.Models.FileEntry> Entries(params string[] paths) =>
        paths.Select(p => new FileMerge.Models.FileEntry { FullPath = p }).ToList();

    [Fact]
    public void A_unique_name_gets_no_hint()
    {
        var entries = Entries(@"C:\a\one.txt", @"C:\b\two.txt");

        DuplicateNameResolver.Apply(entries);

        Assert.All(entries, e => Assert.Null(e.DuplicateHint));
    }

    [Fact]
    public void Repeated_names_are_labelled_with_their_folder()
    {
        var entries = Entries(
            @"C:\work\2023\report.txt",
            @"C:\work\2024\report.txt",
            @"C:\work\draft\report.txt");

        DuplicateNameResolver.Apply(entries);

        Assert.Equal(new[] { "2023", "2024", "draft" }, entries.Select(e => e.DuplicateHint));
    }

    [Fact]
    public void The_label_widens_until_it_tells_them_apart()
    {
        // One folder name is not enough here: both end in "out".
        var entries = Entries(
            @"C:\projects\alpha\out\build.log",
            @"C:\projects\beta\out\build.log");

        DuplicateNameResolver.Apply(entries);

        Assert.Equal(new[] { @"alpha\out", @"beta\out" }, entries.Select(e => e.DuplicateHint));
    }

    [Fact]
    public void A_name_that_stops_repeating_loses_its_hint()
    {
        var entries = Entries(@"C:\a\report.txt", @"C:\b\report.txt");
        DuplicateNameResolver.Apply(entries);
        Assert.NotNull(entries[0].DuplicateHint);

        entries.RemoveAt(1);
        DuplicateNameResolver.Apply(entries);

        Assert.Null(entries[0].DuplicateHint);
    }

    [Theory]
    [InlineData(@"C:\one\two\three", 1, "three")]
    [InlineData(@"C:\one\two\three", 2, @"two\three")]
    [InlineData(@"C:\one\two\three", 9, @"C:\one\two\three")]
    public void Trailing_segments_take_the_end_of_the_path(string path, int count, string expected) =>
        Assert.Equal(expected, DuplicateNameResolver.TrailingSegments(path, count));
}
